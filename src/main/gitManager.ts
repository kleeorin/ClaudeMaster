...
import type { GitStatus, GitFileStatus, GitDiff, GitResult, GitLog, GitBranches } from '../shared/types'
...
// Run git in `dir`. Resolves with stdout; rejects with an Error carrying stderr.
// execFile (not exec) so paths/messages are passed as argv — no shell injection.
// When `dir` is a remote-encoded path, git runs on that host over ssh; the output
// is byte-identical, so all the parsing below is unchanged. ssh relays the remote
// command's exit status, so the `.gitCode === 128` "not a repo" check still works.
async function git(dir: string, args: string[]): Promise<string> {
  const { remoteId, path } = parseTarget(dir)
  if (remoteId) {
    const remote = await remotes.get(remoteId)
    if (!remote) throw wrap('unknown remote', 'ENOENT', '')
    const { stdout, stderr, code } = await ssh.run(remote, ['git', '-C', path, ...args], { maxBuffer: MAX_BUFFER })
    if (code !== 0) throw wrap((stderr || `git exited ${code}`).trim(), code, stdout)
    return stdout
  }
  return new Promise((resolve, reject) => {
    execFile('git', ['-C', path, ...args], { maxBuffer: MAX_BUFFER }, (err, stdout, stderr) => {
      if (err) {
...
// Build the Error shape callers rely on: `.gitCode` distinguishes "not a repo"
// (128) from real failures, and `.stdout` preserves output for `diff --no-index`
// (which exits non-zero *with* the diff on stdout).
  ...
  try {
    ...
    if (untracked && !staged) {
      const out = await git(dir, ['diff', '--no-index', '--', '/dev/null', file]).catch((e: Error) => {
        ...
        // --no-index returns exit 1 *with* the diff — surface it via the message.
        return e.message
      })
