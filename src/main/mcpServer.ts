...
// Hand-rolled (no SDK): the CLI's streamable-HTTP client is happy with plain
// JSON-RPC-over-POST replies — verified against 2.1.198 (initialize →
// notifications/initialized → tools/list → tools/call, Accept:
// application/json,text/event-stream). Bound to loopback; each session gets a
// unique URL token so every tool call is attributable to its origin session
// (needed for "the calling session's active pane", "spawn a subsession of *this*
// session", etc.).
...
export class AppControlMcpServer {
  ...
  // The loopback port the server is listening on (0 until start() resolves). Used
  // to build the remote reverse tunnel (-R port:127.0.0.1:port).
      ...
      switch (msg.method) {
        case 'initialize':
          return reply({
            ...
            capabilities: { tools: {} },
            serverInfo: { name: 'claudemaster-app', version: '0.1.0' },
          })
