...
import type { ConversationMeta, ClaudeEvent } from '../shared/types'

// Claude stores each conversation as a JSONL transcript under
// ~/.claude/projects/<encoded-cwd>/<sessionId>.jsonl. The dir name is the
...
// Noise we hide from a resumed transcript / preview: slash-command wrappers and
// injected caveats/reminders that aren't real conversation.
function isNoise(text: string): boolean {
  const t = text.trimStart()
  return t.startsWith('<command-name>') || t.startsWith('<local-command-caveat>') || t.startsWith('<command-message>')
}

// List resumable conversations for a directory, newest first. Local only for
// now (remote projects live on the remote host — deferred with remote parity).
...
// Read a conversation back as ClaudeEvent[] the renderer can feed through the
// same transcript builder used for live events (assistant / user tool_result).
export async function readConversation(cwd: string, id: string): Promise<ClaudeEvent[]> {
  ...
  for (const line of raw.split('\n')) {
    ...
    if ((t === 'assistant' || t === 'user') && !o.isMeta && !o.isSidechain) {
      // Drop pure command/caveat wrapper turns so the replay reads cleanly.
      if (t === 'user') {
        const txt = contentText((o.message as { content?: unknown })?.content)
