...
// session's working directory (from ~/.claude/projects/<cwd>/*.jsonl) and lets
// you pick one to continue. Picking rebinds the session's engine via --resume
// and replays the transcript into the pane.
export function ResumePicker({
  ...
  useEffect(() => {
    let live = true
    window.api.session.listConversations(cwd).then((l) => { if (live) setList(l) })
    return () => { live = false }
  }, [cwd])
  ...
  return (
    <div className="absolute inset-0 z-30 flex items-start justify-center pt-16 bg-ctp-crust/60" onClick={onClose}>
      <div
        className="w-[min(640px,90%)] max-h-[70%] flex flex-col rounded-lg border border-ctp-surface1 bg-ctp-mantle shadow-xl overflow-hidden"
