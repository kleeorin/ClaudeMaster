...
export function GitPanelView({ sessionId, cwd }: Props) {
  ...
  const loadLog = useCallback(async () => {
    const r = await window.api.git.log(cwd, 100)
    setCommits(r.ok ? r.commits : [])
  }, [cwd])

  // Refresh on mount / session switch, and poll while open so changes Claude
  // makes in the terminal show up without a manual reload. The log only needs
  // pulling while its tab is showing.
  useEffect(() => {
    refresh()
    const t = setInterval(() => {
      refresh()
  ...
  const selectCommit = useCallback(async (hash: string) => {
    ...
    const d = await window.api.git.show(cwd, hash)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    window.api.git.diff(cwd, file.path, sel.staged, file.untracked).then((d) => {
  ...
  const doCommit = useCallback(async () => {
    await run(() => window.api.git.commit(cwd, message))
    setMessage('')
  }, [run, cwd, message])
  ...
  const loadBranches = useCallback(async () => {
    const r = await window.api.git.branches(cwd)
  ...
  const doCheckout = useCallback(async (name: string) => {
    setBranchMenu(false)
    await run(() => window.api.git.checkoutBranch(cwd, name))
    setSelected(null)
    setSelectedCommit(null)
    if (modeRef.current === 'log') loadLog()
  }, [run, cwd, loadLog])
  ...
  const doDeleteBranch = useCallback(async (name: string) => {
    ...
    setError(null)
    try {
      let r = await window.api.git.deleteBranch(cwd, name, false)
      if (!r.ok && /not fully merged/i.test(r.error)) {
        if (window.confirm(`"${name}" isn't fully merged. Force-delete and lose its unmerged commits?`)) {
          r = await window.api.git.deleteBranch(cwd, name, true)
        } else { return }
      }
      ...
      <div className="h-9 shrink-0 flex items-center gap-2 px-3 bg-ctp-mantle border-b border-ctp-surface0">
        ...
        <div className="relative flex-1 min-w-0">
          <button
            ...
            className="w-full flex items-center gap-1 text-xs text-ctp-text disabled:opacity-60"
          >
        ...
        >
          {staged.map((f) => (
            <FileRow
              ...
              onStage={() => run(() => window.api.git.stage(cwd, f.path))}
              onUnstage={() => run(() => window.api.git.unstage(cwd, f.path))}
            />
          ))}
